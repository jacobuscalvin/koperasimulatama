...
<link href='css/ghpages-materialize.css' type='text/css' rel='stylesheet' media='screen,projection'>
    ...
    <h3><b><center>Data Anggota Lancar</b></h3>
	
<div class='row'>
<div class='col s16 m16 l16'>
<div class='card-panel brown lighten-2' style='margin-right: 0rem; margin-left: 4rem;'>	
  ...
  ";
if(empty($_GET['cari'])){
$select = mysql_query ("SELECT a.*,b.*,c.* FROM anggota a INNER JOIN resort b ON a.petugas = b.petugas INNER JOIN unit c ON b.id_unit=c.id_unit WHERE a.status = 'Lancar'");
...
if($_GET['cari'] == 'id'){
$sel = mysql_query("SELECT a.*,b.*,c.* FROM anggota a INNER JOIN resort b ON a.petugas = b.petugas INNER JOIN unit c ON b.id_unit=c.id_unit WHERE a.id_anggota   like '%$_GET[search]%' AND a.status='Lancar'");
}
else if($_GET['cari'] == 'no_ktp'){
$sel = mysql_query("SELECT a.*,b.*,c.* FROM anggota a INNER JOIN resort b ON a.petugas = b.petugas INNER JOIN unit c ON b.id_unit=c.id_unit WHERE a.no_ktp like '%$_GET[search]%' AND a.status='Lancar'");
}
else if($_GET['cari'] == 'nama'){
$sel = mysql_query("SELECT a.*,b.*,c.* FROM anggota a INNER JOIN resort b ON a.petugas = b.petugas INNER JOIN unit c ON b.id_unit=c.id_unit WHERE a.nama  like  '%$_GET[search]%' AND a.status='Lancar'");
}
else if($_GET['cari'] == 'no_kk'){
$sel = mysql_query("SELECT a.*,b.*,c.* FROM anggota a INNER JOIN resort b ON a.petugas = b.petugas INNER JOIN unit c ON b.id_unit=c.id_unit WHERE a.no_kk   like '%$_GET[search]%' AND a.status='Lancar'");
}
else if($_GET['cari'] == 'nama_ibu'){
$sel = mysql_query("SELECT a.*,b.*,c.* FROM anggota a INNER JOIN resort b ON a.petugas = b.petugas INNER JOIN unit c ON b.id_unit=c.id_unit WHERE a.nama_ibu  like '%$_GET[search]%' AND a.status='Lancar'");
}
else if($_GET['cari'] == 'tanggal_input'){
$sel = mysql_query("SELECT a.*,b.*,c.* FROM anggota a INNER JOIN resort b ON a.petugas = b.petugas INNER JOIN unit c ON b.id_unit=c.id_unit WHERE a.tanggal_input  like '%$_GET[search]%' AND a.status='Lancar'");
}
